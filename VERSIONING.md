...
The `0.4.0` line is **`Draft`** (opened 2026-07-05): RFC-ACDP-0015 (Transparency-Log Witness Cosigning) is Draft at version `0.4.0`, promoting the RFC-ACDP-0009 §2.12 reservation (checkpoint witnessing & cosigning). It is a backward-compatible minor: independent witnesses cosign RFC-ACDP-0012 checkpoints so a consumer trusting one honest witness gains split-view protection and an external anchor for checkpoint time. No wire change touches any v0.1.0/0.2.0/0.3.0 body field, JCS rule, content-hash, signature, receipt, or checkpoint semantic — the additions are one new closed schema (`acdp-log-cosignature`), one optional top-level envelope member (`witness_signatures`, a sibling of `log_inclusion`), one new profile (`acdp-log-witness`), and one additive error-enum entry (`invalid_witness_cosignature`); per the namespace rule below the canonical schemas stay in the `v0.1.0` namespace with additive edits. RFC-ACDP-0015's `Version:` header reads `0.4.0-draft`. The line goes Final once the 0.4.0 conformance pack (`wit-*`) passes against two independent interoperating implementations, per the same gate as 0.2.0/0.3.0 (see [CHANGELOG.md](CHANGELOG.md)).
