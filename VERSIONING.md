...
The `0.3.0` line (**core-profile revision**) is in **`Draft`**: RFCs 0003/0007/0008 carry Draft amendments marked *(0.3.0)* in-prose (their `Version:` headers read `0.3.0-draft`; passages marked *(0.2.0)* remain Draft from the 0.2.0 line). It comprises one **conformance tightening** — `Idempotency-Key` support (RFC-ACDP-0003 §6, including the §6.2.2 atomic storage contract and the §6 TTL bounds) is REQUIRED for `acdp-registry-core` when the registry advertises `acdp_version` ≥ 0.3.0; registries advertising 0.1.0/0.2.0 are unchanged — and one **backward-compatible addition**, the OPTIONAL `limits.max_publish_per_minute` capabilities field (RFC-ACDP-0007 §3.2). Neither is a wire change: no body field, JCS rule, content-hash, signature semantic, header syntax, or error code is touched, and per the namespace rule below the canonical schemas stay in the `v0.1.0` namespace with one additive edit (the closed `limits` sub-object gains the optional field — the same posture as `registry_receipt` on the closed publish response in 0.2.0). Migration: a 0.2.0-conformant registry upgrades by implementing RFC-ACDP-0003 §6 *before* advertising `acdp_version` ≥ 0.3.0; until then it keeps advertising `0.2.0` and remains fully conformant.
