...
├── fed-*.json      Cross-registry / SSRF protection scenarios (§7 SSRF; §4.1 resolution + traversal; fed-009 receipts, RFC-ACDP-0010 §11)
...
| `vis-005` | Private + audience: audience member MUST NOT find context via search/`derived_from` filter; search visibility for `private` is strictly `agent_id`-only | mixed (per-scenario; `matches` counts pinned, `total_estimate` pinned as a leak-invariance property, not an exact value) |
...
| `dk-002` | Three cases, in fixture order: a character outside the base58-btc alphabet; a non-`z` multibase prefix; and `z6Mk`, whose payload decodes cleanly to 2 bytes and is therefore a **step-3** rejection, not a malformed-multibase one | failure: `key_resolution_failed` (`schema_violation` tolerated for cases 1-2 only — see the note below) |
...
| `dk-004` | `signature.key_id` fragment ≠ the DID's method-specific identifier (`did:key:z<mb>#z<mb>` convention) | failure: `key_resolution_failed` (`schema_violation` tolerated — see the note below) |

did:key resolution is **pure** — no network, no DID document, no `assertionMethod` check (the DID *is* the key). **Which rejections may be `schema_violation`:** `dk-002` and `dk-004` carry an `expected.alternative_error_code` of `schema_violation`, tolerated for a registry whose request validation enforces a stricter `did:key` grammar than the deliberately loose `acdp-common` `did`/`did_url` patterns and so catches them before the resolver, because they fault the *encoding* of the identifier rather than the key material it encodes. On `dk-002` the alternative is scoped by `expected.alternative_applies_to_cases` to **cases 1 and 2 only**: case 3 (`z6Mk`) decodes cleanly to two bytes and is rejected at step 3, where `schema_violation` is forbidden. `dk-001` deliberately carries **no** alternative — not because it is harder to detect (the multicodec prefix is stable in base58-btc, so a curve allowlist catches it lexically), but because its fault is the *key material* rather than the *encoding*: an unsupported curve, whose remedy is to re-key rather than to fix the request. Widening it would also leave nothing verifying that an implementation can tell `0xe701` from `0xed01`. See RFC-ACDP-0001 §5.11.1, "Which failures are resolution failures". The golden path is `sig-003`. `dk-001`/`dk-002`/`dk-004` are required for registries advertising `did:key` and for all 0.2.0 consumers; `dk-003` is required for 0.2.0 registries that do not advertise it.
...
| `fed-011` | A resolved body's `ctx_id` does not equal the requested `ctx_id` (RFC-ACDP-0006 §4.1 step 7) even though the body's signature and `content_hash` both verify — `ctx_id` sits outside the RFC-ACDP-0001 §5.7 exclusion set, so neither check can catch a substituted identity | failure: `cross_registry_resolution_failed` |

These fixtures are required for `acdp-registry-federated` profile conformance (registries/profiles.md). The bundled conformance runner does not execute them; they describe black-box scenarios that registry integration tests MUST verify against a live deployment. `fed-007` pins the mixed-answer rejection rule (RFC-ACDP-0006 §7.1), `fed-008` the same-authority redirect definition (RFC-ACDP-0006 §7.5), `fed-010` the partial-walk reporting rule (RFC-ACDP-0006 §4.1 — truncation is not completeness), and `fed-011` the §4.1 step 7 resolved-identity binding (`body.ctx_id` MUST equal the requested `ctx_id`, on pain of `cross_registry_resolution_failed`, even when signature and `content_hash` both verify). `fed-009` is required when the resolver also advertises `acdp-registry-receipts` or resolves from receipts-advertising upstreams; `fed-010` is additionally required for `acdp-consumer` implementations that walk `derived_from`, whether or not they resolve cross-registry; `fed-011` is additionally required for `acdp-consumer` implementations that resolve `acdp://` references, whether or not they walk `derived_from`.
