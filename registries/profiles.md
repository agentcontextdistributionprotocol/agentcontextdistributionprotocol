...
| **Conformance fixtures** | All `can-*` (including `can-008`, `can-009` for forward-compat hash verification), `sig-001`, `caps-001..006` (capabilities validation), `status-001..004` (registry-state status pattern tolerance), `schema-001..004` (schema openness), `body-001..002` (consumer MUST reject a retrieved body whose `origin_registry` is a DID rather than a hostname — RFC-ACDP-0002 §3.1), the consumer side of `vis-003`, `pub-007` and `schema-002` (publish-response shape), `pub-010` (non-`did:web` contributor tolerance), plus any cross-registry traversal vectors a consumer follows |
