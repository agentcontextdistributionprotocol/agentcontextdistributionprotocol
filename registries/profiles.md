...
| | Required |
|---|---|
| **Behavioral requirements** | End-to-end signature verification (RFC-ACDP-0001 §5.11) on every retrieved context, including pure `did:key` resolution (§5.11.1) for `did:key` producers *(0.2.0)*; registry-receipt verification per RFC-ACDP-0010 §8 whenever a `registry_receipt` is present, with the receipt verdict reported separately from the body verdict *(0.2.0)*; `acdp://` cross-registry resolution per RFC-ACDP-0006 if followed (with SSRF protections of §7 if performed server-side); local visibility re-verification per RFC-ACDP-0008 §4.5; tolerance of unknown body / registry-state fields (RFC-ACDP-0001 §6, RFC-ACDP-0004 §4.1, RFC-ACDP-0007 §3.3) |
