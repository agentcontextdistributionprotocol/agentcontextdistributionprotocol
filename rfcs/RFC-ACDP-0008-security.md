...
| Timestamp authenticity (`created_at`) | **Low** — assertion only | **Medium** — non-repudiably attested at mint; mint-time backdating remains possible where no transparency log is deployed (RFC-ACDP-0009 §2.11, *(0.3.0)* since shipped as [RFC-ACDP-0012](RFC-ACDP-0012-transparency-log.md) — see below) |

***(0.3.0)*** The transparency log has shipped as [RFC-ACDP-0012](RFC-ACDP-0012-transparency-log.md) (optional profile `acdp-registry-transparency-log`). Where the serving registry advertises it and the consumer (or a monitor it trusts) retains checkpoints, the residual mint-time limitations become **detectable-with-witnesses**: post-mint backdating is bounded by a leaf's position relative to retained checkpoints, history rewrites fail consistency proofs (RFC-ACDP-0012 §9.2), omission of a receipt-bearing publish is provable by its producer (a receipt with no obtainable inclusion proof), and split-view requires signing conflicting checkpoints — compact, non-repudiable evidence. Timestamp authenticity remains **Medium** even then: checkpoint `timestamp` values are still registry-asserted, and *(0.4.0)* external time anchoring is specified as witness cosigning by [RFC-ACDP-0015](RFC-ACDP-0015-witness-cosigning.md) (promoting the RFC-ACDP-0009 §2.12 reservation). Detection requires someone to retain and compare checkpoints — a consumer that never does gets the 0.2.0 receipt guarantees unchanged (RFC-ACDP-0012 §13).
...
The protocol-native next layer — an append-only, registry-signed publication log with Merkle-tree checkpoints — entered as the [RFC-ACDP-0009 §2.11](RFC-ACDP-0009-extensions.md#211-transparency-log--promoted-to-rfc-acdp-0012-acdp030) reservation; ***(0.3.0)*** it has since shipped as [RFC-ACDP-0012](RFC-ACDP-0012-transparency-log.md).
...
- [RFC-ACDP-0009 Extensions](RFC-ACDP-0009-extensions.md) — §2.11 (transparency-log reservation, since promoted to RFC-ACDP-0012).
