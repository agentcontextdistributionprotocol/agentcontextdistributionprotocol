...
**Which failures are resolution failures (NORMATIVE).** Of the six steps above, **steps 1 through 4 are resolver steps**: a failure at any of them is `key_resolution_failed`, whether or not detecting it happened to require decoding. The canonical `did`/`did_url` patterns in `acdp-common.schema.json` are deliberately loose and delegate method-specific validation to this algorithm, so a schema-conformant validator does not reject the cases these fixtures pin at step 1 of the publish pipeline. (The patterns do exclude some characters outright, so a validator can reject *some* malformed identifiers there; what it cannot do is adjudicate multibase, multicodec, or fragment consistency.)

A registry whose request validation enforces a **stricter `did:key` grammar** than those patterns MAY surface `schema_violation` for a failure of **step 1 or step 2** — the steps that decide whether the string is a well-formed `did:key` at all (fragment ≠ method-specific identifier; a multibase prefix other than `z`, or a character outside the base58-btc alphabet). Note that a payload which is merely *short* is not a step-2 failure if it decodes — `did:key:z6Mk` decodes cleanly to two bytes and is rejected at step 3 for its prefix, so it takes no `schema_violation` carve-out; `dk-002` case 3 pins exactly that. Such a registry is being defensive, not non-conformant, and the `did:web` precedent is the same: a missing `key_id` fragment is a permanent *resolution* failure (§5.11) even though it is a pure string check.

It MUST NOT report `schema_violation` for **step 3 or step 4**. The line is not whether a fault happens to be detectable without decoding — it often is: the multicodec prefix is stable in the base58-btc encoding, so `ed25519-pub` keys always render `z6Mk…` and a 32-byte `secp256k1` key always renders `z6Dt…`, and a grammar carrying a curve allowlist rejects the latter having decoded nothing. The line is **what the fault is about**. Steps 1 and 2 fault the *encoding of the identifier*; steps 3 and 4 fault the *key material it encodes* — an unsupported curve, or a length that does not match the curve. The two classes are distinguishable without reference to any implementation: steps 1 and 2 are stated wholly over characters of the identifier, while steps 3 and 4 cannot be stated without referring to the decoded bytes and a curve parameter. Typically the remedies differ too, and the error code is what signals which — `schema_violation` reads as "your request is malformed, fix it", `key_resolution_failed` as "this key cannot be used, re-key" — though that is an illustration rather than the rule: a short payload under a valid prefix (step 4) is often a re-encoding fault. A registry that encodes a curve allowlist in its grammar is free to do so, but the resulting rejection is still a step-3 verdict and MUST be reported as one. Note it is also not `unsupported_algorithm`: `signature.algorithm` may be perfectly supported — it is the identity encoding that is unusable.

Fixtures record the boundary: `dk-004`, and `dk-002` cases 1-2, carry an `alternative_error_code` of `schema_violation`. `dk-001` deliberately does not, because a conformance harness that accepts any rejection for it cannot distinguish an implementation that understands `0xe701` from `0xed01` from one that never looked — and neither does `dk-002` case 3, which despite its family name is a step-3 rejection rather than a multibase failure.
...
|---|---|---|
| `StrictV010` | The full §5.11 strict pipeline: schema validation, `content_hash` recomputation, `did:web` resolution, signature verification, and `embedded.content_hash` verification (the field inside `embedded`, RFC-ACDP-0002 §6.3 — not the DataRef-root `content_hash` of §6.1, which this pipeline does not verify: for `location`-form data its check needs a fetch, and for embedded data RFC-ACDP-0002 §6.6 leaves it to the registry's discretion — a MAY — rather than to this consumer pipeline). Returns on the first failure. | **Yes** — the only mode covered by the `acdp-consumer` conformance profile. |
