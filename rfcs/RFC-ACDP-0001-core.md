...
3. **Check the multicodec prefix.** The decoded bytes MUST begin with `0xed 0x01` (Ed25519 public key) or `0x12 0x00` (P-256 public key). Any other prefix MUST be rejected with `key_resolution_failed`.
...
5. **Check algorithm consistency.** `signature.algorithm` MUST match the multicodec-implied algorithm (`ed25519` for `0xed01`, `ecdsa-p256` for `0x1200`); mismatch MUST be rejected with `invalid_signature` (the same algorithm-binding rule as step 6 of the `did:web` algorithm).
