...
7. **Walk further references.** For each entry in `body.derived_from`, repeat from step 1 if the consumer needs the predecessor. ACDP's content-addressing forbids cycles in honest data (a body cannot reference its own future `ctx_id`), but consumers SHOULD detect cycles defensively (track visited `ctx_id`s within a single walk) and abort with a logged error if one is observed — its presence indicates a tampered body or a registry serving forged data.
   ...
   **Partial-walk reporting — truncation is not completeness.** Whenever any of the four controls above (depth, total nodes, fanout, timeout) cuts a `derived_from` walk short and the implementation surfaces a partial result rather than aborting outright, it MUST distinguish that truncated result from a complete one in its result surface — e.g. a `complete: false` flag, an explicit truncation marker, or a typed partial-result indication accompanying the nodes already fetched and verified — and it MUST NOT present a truncated evidence set as exhaustive. The marker SHOULD identify which control fired and, where practical, the frontier (the `ctx_id`s whose subtrees were not walked). Rationale: cross-registry walks exist to assemble the evidence chain behind a conclusion (§2); a consumer handed N verified predecessors with no truncation signal will treat them as the *complete* evidence basis, when the unwalked remainder may contain exactly the disputing or superseding context the evaluation needed. Silent truncation therefore converts a resource-bounding control into an integrity failure. Implementations that prefer to fail closed MAY instead abort the whole walk with `cross_registry_resolution_failed` (this remains conformant, and for fanout overruns remains the preferred behavior per the table above); what is non-conformant is returning a truncated result that is indistinguishable from a complete one. Conformance fixture `fed-010` pins the success-with-truncation-marker path.
