...
**(NORMATIVE) Check 8 is scoped to `embedded.content_hash` only.** A DataRef-root `content_hash` (§6.1) present on an *embedded* DataRef is **not** part of check 8 and a registry is NOT required to verify it at publish. The root field's generic wording covers embedded data (§6.3), so the two may legitimately both be present and both cover the same decoded bytes — but only `embedded.content_hash` carries a publish-time registry obligation. A registry MAY additionally verify a root `content_hash` over decoded embedded bytes and reject a mismatch with `data_ref_hash_mismatch`; it MUST NOT reject a publish merely because a root `content_hash` is present alongside an embedded one. Consumers verify the root field after fetching `location`-form data (§6.5); for embedded data there is nothing to fetch, so the check is the registry's or nobody's.
