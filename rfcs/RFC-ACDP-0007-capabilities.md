...
This document is a Final ACDP specification (acdp/0.1.0, with Final amendments through acdp/0.4.0). It is stable for the released lines; subsequent breaking changes require a new RFC and a version bump per [VERSIONING.md](../VERSIONING.md).

Passages marked *(0.2.0)* are amendments from the acdp/0.2.0 Trust & Hardening program. Passages marked *(0.4.0)* are amendments from the acdp/0.4.0 witness-cosigning program (the `invalid_witness_cosignature` error code, RFC-ACDP-0015 §10). Passages marked *(0.3.0)* are amendments from the acdp/0.3.0 program (mandatory Idempotency-Key support per RFC-ACDP-0003 §6.4, the OPTIONAL `limits.max_publish_per_minute` capabilities field, and the RFC-ACDP-0011/0012/0013 surfaces). Both lines are **Final** as of 2026-07-05, promoted after their conformance packs passed against two independent interoperating implementations (see [CHANGELOG.md](../CHANGELOG.md)); the markers record provenance, not status. Neither set changes any v0.1.0 body field, hash, or signature semantic; everything not so marked remains Final and wire-frozen for acdp/0.1.0.
...
`Content-Type` MUST be `application/acdp+json`. The HTTP status code is per the table in §5. The error envelope MUST be returned for **every** failure response from an ACDP endpoint, including 4xx, 5xx, 415 Unsupported Media Type *(0.5.0)*, 501 Not Implemented, and 502 Bad Gateway responses. Registries MUST NOT return empty bodies, framework default error pages, or non-`application/acdp+json` content types on ACDP endpoints. The corresponding fixture is `err-001-internal-error.json` (illustrating a 500 envelope).

### 4.1 Request media type *(0.5.0)* (NORMATIVE)

The rules above govern the media type a registry **emits**. This section governs the one it **accepts**, which had no normative statement before 0.5.0.

On a **body-bearing** request (`POST`, and any future method carrying a request body), a registry:

- **MUST** accept `application/acdp+json` — the canonical type registered in [`registries/media-types.md`](../registries/media-types.md) (RFC-ACDP-0001 §5.1).
- **MAY** accept `application/json`, and MAY accept any other media type it chooses. RFC-ACDP-0001 §5.1 permits the former explicitly, and `registries/media-types.md` RECOMMENDS it as a legacy-client fallback; neither permission is narrowed here. A registry's accept-set is its own choice beyond the one mandatory member.
- **MUST** ignore media-type **parameters** when deciding acceptance. `application/acdp+json; charset=utf-8` is the same media type as `application/acdp+json` for this comparison — RFC-ACDP-0001 §5.1 already fixes the encoding as UTF-8, so a `charset` parameter is redundant rather than distinguishing. Comparison is case-insensitive on the type and subtype, per [RFC 9110 §8.3.1].
- **MUST**, if and only if it advertises `acdp_version` ≥ `0.5.0`, reject a request whose `Content-Type` is outside its accept-set with `unsupported_media_type` (HTTP 415) and the standard error envelope. The body MUST NOT be parsed first: the point of the code is that no structural claim about the body is being made.
- **MAY** choose either behavior when `Content-Type` is **absent** on a body-bearing request — reject with `unsupported_media_type`, or infer `application/acdp+json` and proceed. Registries SHOULD document which they do. A missing header is a client defect either way, and mandating one reading would break deployed clients for no security gain.

**Which of the above binds which version (NORMATIVE).** Only the fourth bullet — the MUST to answer `unsupported_media_type` — is new behavior, and it is gated at `acdp_version` ≥ `0.5.0` because it *adds a rejection*, which a clarification to a Final line may never do.

The first three bullets bind **every version**, because none of them adds a rejection and none of them is new:

- "MUST accept `application/acdp+json`" restates `registries/media-types.md`, which has said since v0.1.0 that "All ACDP endpoints accept and emit this type", and RFC-ACDP-0001 §5.1.
- "MAY accept `application/json` and anything further" restates RFC-ACDP-0001 §5.1's existing MAY verbatim in effect; it grants latitude and withdraws none.
- "MUST ignore media-type parameters" *forbids* a rejection rather than adding one, and makes coherent a permission `registries/media-types.md` already grants — implementations MAY send `; charset=utf-8`, which is only meaningful if recipients tolerate it. A registry that rejects `application/acdp+json; charset=utf-8` was never conformant with that permission; this states plainly what was already implied.

The fifth bullet's *latitude* (an absent `Content-Type` is the registry's choice) likewise binds every version; only the specific code named in its reject arm is 0.5.0-gated, since no earlier version may emit it.

This check precedes every step of the RFC-ACDP-0003 §2.1 publish pipeline, including its step 1 schema validation — see that section's pre-step.

**GET and other body-less methods are out of scope**: with no request body there is no request media type to reject. Content negotiation on responses is governed by the `Accept` header (`registries/media-types.md`), which is unchanged.

**Method-level rejection (405) is deliberately out of scope and no code is minted for it.** ACDP maintains a media-types registry, requests IANA registration for its type (RFC-ACDP-0001 §11.2), and carries normative acceptance prose — so the media type is protocol surface a registry governs, and 415 is adjudicable against the spec. ACDP registers no method vocabulary and states nothing about method routing, so a 405 is a framework concern with no ACDP-level rule to violate. A 405 is therefore **not** "a failure response from an ACDP endpoint" in the sense of §4: the request never reached an ACDP operation, so §4's envelope requirement does not attach and no `error.code` is owed. Implementations MAY answer 405 however their framework does. They MUST NOT invent an ACDP code for it, and MUST NOT reuse `internal_error` (which is reserved for genuine internal faults and is in consumers' retryable set). An implementation that prefers envelope-shaped 405s for consistency is free to do so, but that is a local choice outside this specification, not a conformance requirement — §4's `error.code` is REQUIRED and drawn from the closed §5 enum, so there is deliberately no conformant ACDP code to place in such a body.

[RFC 9110 §8.3.1]: https://www.rfc-editor.org/rfc/rfc9110#section-8.3.1
...
| `data_ref_hash_mismatch` | 400 | A DataRef's bytes do not match the producer-declared hash. Two distinct, independent fields can trigger this, and a DataRef MAY carry both. Returned by a registry at publish time when **`embedded.content_hash`** — the optional member inside the closed `embedded` sub-object (RFC-ACDP-0002 §6.3) — does not match the decoded `embedded.content` (RFC-ACDP-0002 §6.6 Check 8; fixture `data-ref-007`). Also the code a consumer SHOULD surface when it fetches an external `data_ref.location` and the retrieved bytes do not match the **DataRef-root** `content_hash` (RFC-ACDP-0002 §6.1, §6.5; fixture `data-ref-008`). The body's own `content_hash` and signature are still valid — the integrity failure is at the data-reference level, not the body level. | RFC-ACDP-0002 §6.5, §6.6 |
...
| `key_resolution_failed` | 400 | The signing key referenced by `signature.key_id` could not be resolved due to a permanent condition: the DID document parsed successfully but does not contain the requested key fragment; the fragment is missing from `key_id`; or the producer DID resolves to a network target forbidden by SSRF policy (RFC-ACDP-0008 §4.8). For `did:key`, resolution is pure (RFC-ACDP-0001 §5.11.1) and the same code covers its step 1-4 failures: a `signature.key_id` fragment that does not byte-equal the method-specific identifier, a method-specific identifier that is not valid base58-btc under the `z` multibase prefix, a multicodec prefix that is neither `0xed01` nor `0x8024`, or a key-byte length that does not match the curve — none of which involves a DID document or a network fetch. Fixtures `dk-001`, `dk-002`, `dk-004`; `dk-002` (cases 1-2 only) and `dk-004` tolerate `schema_violation` from a registry whose stricter grammar rejects the *encoding*; `dk-001` does not, because its fault is the *key material* (an unsupported curve), whose remedy is to re-key rather than to fix the request — and neither does `dk-002` case 3 (`z6Mk`), which decodes cleanly and is likewise a step-3 rejection. Producer error; not retryable. | RFC-ACDP-0003 §2.1 step 6, RFC-ACDP-0008 §4.8 |
...
| `invalid_witness_cosignature` *(0.4.0)* | 502 | A transparency-log witness cosignature failed the RFC-ACDP-0015 §8 verification procedure: a closed-schema parse failure, a witness-key signature that does not verify over the recomputed cosignature preimage, a `signature.key_id` whose DID portion ≠ `witness_id`, a `witnessed_checkpoint` tuple that does not match the checkpoint being evaluated, or a future-dated `witnessed_at` beyond the skew allowance. Deliberately **not** `invalid_log_proof`: a cosignature failure indicts a *witness's attestation* (an independent signer), not the registry's *log*, and the verdicts are independent (RFC-ACDP-0015 §8, RFC-ACDP-0012 §9.3). Emitted on the wire by a registry aggregating cosignatures, or a resolver validating a witness's cosignatures on a caller's behalf — hence 502, the upstream party is at fault; also the verification-failure category consumer SDKs use for a locally failing cosignature. A merely *stale* (but valid) cosignature is consumer freshness policy, not this code; an un-witnessed (0-witnessed) checkpoint is not an error. | RFC-ACDP-0015 §8, §10 |
| `unsupported_media_type` *(0.5.0)* | 415 | The request `Content-Type` on a body-bearing method is a media type this registry does not accept (see §4.1). The request body was **not** parsed, so no structural claim about it is made — this is deliberately not `schema_violation`, which asserts a validation that did not run and is pinned to 400. The client's remedy is to resend the identical request under an accepted media type. | RFC-ACDP-0001 §5.1, RFC-ACDP-0007 §4.1 |
...
- `data_ref_hash_mismatch` — a DataRef's bytes do not match the producer-declared hash: fetched (external) bytes against the DataRef-root `content_hash` (RFC-ACDP-0002 §6.1), or decoded (embedded) bytes against `embedded.content_hash` (RFC-ACDP-0002 §6.3). These are two independent fields and a DataRef MAY carry both. The **body itself is cryptographically valid**; only the referenced data has diverged from what the producer signed. Returning `hash_mismatch` here would wrongly imply the whole body failed verification; returning `invalid_signature` would wrongly imply a signature-verification failure.
...
- Report `data_ref_hash_mismatch` when a DataRef's bytes do not match the producer-declared hash. Two distinct, independent fields can trigger this, and a DataRef MAY carry both: bytes fetched from an external `data_ref.location` are checked against the **DataRef-root** `content_hash` (RFC-ACDP-0002 §6.1, §6.5 — consumer-side, at fetch time), while bytes decoded from `data_ref.embedded` are checked against **`embedded.content_hash`**, the optional member inside the closed `embedded` sub-object (RFC-ACDP-0002 §6.3, §6.6 Check 8 — registry-side, at publish time). The root field is not *restricted* to `location`-form data — §6.1's generic wording covers embedded data too — but only `embedded.content_hash` is what §6.6 Check 8 makes a registry verify at publish.
