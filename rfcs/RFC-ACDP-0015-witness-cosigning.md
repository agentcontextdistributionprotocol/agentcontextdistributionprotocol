...
Vectors are generated by `scripts/gen-0.4.0-vectors.py` (never hand-written), following `scripts/gen-0.3.0-vectors.py`; the generator uses the same `jcs` + `cryptography` libraries as the runner and the `log-001` golden checkpoint as the cosigned material.
