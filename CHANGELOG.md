...
### Phase A — semantic similarity cut from v0.0.1

Seven rounds of review from two independent sources reached the same conclusion: similarity adds 89 RFC lines, 2 schemas, 2 body fields, 4 capability fields, 1 error code, and 1 publish validation step for a feature whose gating (capability-document check, optional within `acdp-registry-discovery`) was correct but whose cognitive load and surface area were disproportionate to v0.0.1's "minimal substrate" goal. Cut now before any implementation depends on it. Reserved cleanly for v0.1 in RFC-ACDP-0009 §2.9.

The cut is one logical change across 27 files plus 3 deletions, shipped in a single commit. `make validate` 29/29 + 14/14 conformance vectors green after the cut. No conformance fixture referenced similarity.

**Deleted (3 files):** `schemas/json/acdp-similarity-request.schema.json`, `schemas/json/acdp-similarity-response.schema.json`, `examples/search/similarity-response.json`.

**Schema removals:** `embedding` and `embedding_model` from `acdp-context-body.schema.json` and `acdp-publish-request.schema.json` (and their conditional `embedding_model required when embedding present` allOf clauses); `supported_embedding_models`, `previously_supported_embedding_models`, `limits.max_embedding_dimensions`, `limits.max_top_k` from `acdp-capabilities.schema.json`; `unsupported_embedding_model` from `acdp-error.schema.json` enum; `embedding` and `embedding_model` `$defs` from `acdp-common.schema.json`; `similarity_response` entry from `acdp-index.schema.json`. Reserved-codes description on `acdp-error.schema.json` updated to flag both `immutable_field` and `unsupported_embedding_model` as v0.1+ reservations.

**RFC removals:** RFC-0005 §3 "Semantic Similarity" deleted entirely (~90 lines: §3 intro, §3.1–3.5); §4–§7 renumbered to §3–§6; §1 intro now states "one discovery modality: keyword search"; errors table loses `unsupported_embedding_model` and `not_implemented` rows; security considerations no longer reference embeddings. RFC-0001 §3 item 4 ("keyword and semantic discovery" → "keyword discovery"); §7 transport endpoint list drops `GET/POST /contexts/similar`; §9.1 `acdp-registry-discovery` profile loses the similarity bullet. RFC-0002 §3.6 loses `embedding` and `embedding_model` rows; §7 security note about embeddings removed. RFC-0003 §2.1 step 8 "Embedding-model check" removed and steps 9–14 renumbered to 8–13 (boundary text "steps 1–8" → "steps 1–7", "Steps 9–13 atomic" → "Steps 8–12 atomic", "reach step 12" → "step 11"); implementer note loses "embedding-model availability"; §5 errors table loses the embedding-model row. RFC-0004 §8 loses the embedding/similarity bullet. RFC-0007 capabilities §3.2 loses 4 rows (similarity caps + max_embedding_dimensions + max_top_k); §3.4 example trimmed; §5 error registry loses `unsupported_embedding_model` row, gains `unsupported_embedding_model` to the reserved-codes note; `not_implemented` source citation now points at RFC-0007 §4 instead of the removed RFC-0005 §3. RFC-0008 §3 threats table renumbered (3.5 "Visibility leakage via similarity" removed, 3.6–3.11 → 3.5–3.10); §6 mention of "similarity" in auth contexts removed; §7.6 attack scenario removed; §8 summary "embeddings are not exempt" removed. RFC-0009 §2.9 "Semantic Similarity and Embeddings (likely v0.1)" added with full reservation (body fields, endpoints, capabilities, error code, privacy requirements, model retirement semantics, profile name `acdp-registry-similarity`).

**Registry/example/script/doc removals:** `registries/error-codes.md` `unsupported_embedding_model` row removed; `not_implemented` source citation updated. `examples/capabilities/acdp-capabilities.json` loses `supported_embedding_models` and `limits.max_embedding_dimensions`/`max_top_k`. `examples/retrieval/coauthored-analysis-context.json` loses `embedding`/`embedding_model`. `examples/README.md` file tree and validation table updated. `scripts/validate-json.sh` simplified — drops `SIMILARITY_RESPONSE_SCHEMA` constant and the search-dir `case` branch; `search/` now uses the standard `validate_dir_against` helper. Docs (architecture.md pipeline + reading-order, overview.md RFC table, non-goals.md, threat-model.md threats + scenario 6, integration-guide.md similarity example) updated consistently — explicitly out of the plan's strict grep but included to prevent new drift.
