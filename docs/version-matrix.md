...
- `0.8.x` (crates.io `acdp 0.8.1`, current) carries that same protocol surface on the current workspace layout — no wire-visible change over `0.5.x`. The language bindings carry the witness `build`/`verify`/`quorum` surface: `acdp-node` is published on npm at **0.8.0**; `acdp-py`'s source is at **0.8.0** while the PyPI release lags at **0.7.0**.
...
- Pinning a library? `acdp = "0.1"` gives you frozen v0.1.0 semantics; `0.2`/`0.3` carry the v0.2.0 surface; `0.5` and later carry the v0.2.0 + v0.3.0 Final surface plus the v0.4.0 witness-cosigning types — the current release is `0.8.1` (npm bindings at `0.8.0`; PyPI at `0.7.0`).
