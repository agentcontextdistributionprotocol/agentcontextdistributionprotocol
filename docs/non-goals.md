...
**What ACDP does instead.** Supersession: a producer publishes a corrected v2 with `supersedes: ctx_id_of_v1`. Original v1 stays in the registry but `status: superseded`. RFC-ACDP-0009 reserved a formal lifecycle-events mechanism for a future version; **0.3.0 specifies it** as [RFC-ACDP-0013](../rfcs/RFC-ACDP-0013-lifecycle-events.md) (Draft, OPTIONAL `acdp-registry-lifecycle` profile) — and it answers the questions above conservatively (producer or registry only; signed events; whole-context only) while preserving this section's invariant: retraction is **mark-not-delete**, the body stays permanent and retrievable. For what permanence means under erasure-rights regimes (GDPR and similar), see [docs/data-protection.md](data-protection.md).
...
**What ACDP does instead.** Supersession for corrections; visibility-restriction for access control over time. (Storage-cost concerns for very old bodies are a deployment problem, not a protocol one.) For the data-protection consequences — carry-by-reference for erasable data, and how a deployment-policy hard delete must be surfaced — see [docs/data-protection.md](data-protection.md).
