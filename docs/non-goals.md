...
**What ACDP does instead.** Supersession: a producer publishes a corrected v2 with `supersedes: ctx_id_of_v1`. Original v1 stays in the registry but `status: superseded`. RFC-ACDP-0009 reserves a formal lifecycle-events mechanism for a future version. For what permanence means under erasure-rights regimes (GDPR and similar), see [docs/data-protection.md](data-protection.md).
