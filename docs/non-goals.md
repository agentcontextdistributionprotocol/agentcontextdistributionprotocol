...
**What ACDP does instead.** Supersession: a producer publishes a corrected v2 with `supersedes: ctx_id_of_v1`. Original v1 stays in the registry but `status: superseded`. RFC-ACDP-0009 reserved a formal lifecycle-events mechanism for a future version; **0.3.0 specifies it** as [RFC-ACDP-0013](../rfcs/RFC-ACDP-0013-lifecycle-events.md) (Final, OPTIONAL `acdp-registry-lifecycle` profile) — and it answers the questions above conservatively (producer or registry only; signed events; whole-context only) while preserving this section's invariant: retraction is **mark-not-delete**, the body stays permanent and retrievable. For what permanence means under erasure-rights regimes (GDPR and similar), see [docs/data-protection.md](data-protection.md).
